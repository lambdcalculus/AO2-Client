...
void AOEmoteButton::set_image(QString p_chr, int p_emote_number, bool p_enabled)
{
  ...
  QString texture_path = ao_app->get_character_path(
      p_chr, QString("emotions/button%1_off.png").arg(true_emote_number));
  ...
  if (p_enabled)
  {
    const QString enabled_texture_path = ao_app->get_character_path(
        p_chr, QString("emotions/button%1_on.png").arg(true_emote_number));
    ...
    {
      const QString selected_texture_path =
          ao_app->get_character_path(p_chr, "emotions/selected.png");

      if (file_exists(selected_texture_path))
      {
        w_selected->setStyleSheet(
            QString("border-image: url(%1)").arg(selected_texture_path));
      }
      else
      {
        w_selected->setStyleSheet(
  ...
  setText(texture_exist ? QString()
                        : ao_app->get_emote_comment(p_chr, p_emote_number));
  setStyleSheet(texture_exist ? QString("%1 { border-image: url(%2); }")
