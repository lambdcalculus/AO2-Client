...
void Courtroom::on_evidence_name_edited()
{
  ...
  ao_app->send_server_packet(DRPacket("EE", f_contents));
}
...
void Courtroom::on_evidence_clicked(int p_id)
{
  ...
  {
    ao_app->send_server_packet(DRPacket("PE", {"<name>", "<description>", "empty.png"}));
    return;
  }
...
void Courtroom::on_evidence_delete_clicked()
{
  ...
  ao_app->send_server_packet(DRPacket("DE", {QString::number(current_evidence)}));
